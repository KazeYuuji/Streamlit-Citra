...
with tab1_label:
    ...
    plt.close()
    with st.expander("ℹ️ Tentang Model Warna RGB"):
        st.markdown("""
        **RGB (Red, Green, Blue)** adalah model warna aditif. Setiap pixel memiliki 3 channel 
        dengan nilai 0–255 (8 bit), total 16,7 juta warna. Citra digital disimpan sebagai 
        matriks 3D: (tinggi × lebar × 3).
        """)
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    show_full = st.checkbox("Tampilkan resolusi penuh", value=False)
    rh, rw = img_gray.shape
    if not show_full and (rh > 20 or rw > 20):
        step = max(1, rh // 20, rw // 20)
        display_matrix = img_gray[::step, ::step]
        st.info(f"Sampling setiap {step} pixel untuk performa ({display_matrix.shape[0]}×{display_matrix.shape[1]})")
    else:
...
with tab5_label:
    ...
    with col_right:
        ...
        use_manual = st.checkbox("Gunakan threshold manual", value=False)
        if use_manual:
            manual_t = st.slider("Nilai threshold", 0, 255, 127)
            _, manual_bin = cv2.threshold(blur_gray, manual_t, 255, cv2.THRESH_BINARY)
            st.image(manual_bin, caption=f"Manual threshold={manual_t}", use_column_width=True, clamp=True)
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    if len(filtered) > 0:
        ...
        with st.expander("📏 Estimasi Area & Volume (Real World)"):
            ...
            img_w_cm = c_a.number_input("Lebar citra nyata (cm)", 1.0, 1000.0, 100.0,
                                         help="Perkiraan lebar sungai yang tercakup dalam citra")
            thickness = c_b.number_input("Ketebalan sampah (mm)", 0.1, 50.0, 1.0,
                                          help="Estimasi ketebalan rata-rata sampah plastik")
