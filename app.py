...
@safe_render
def render_tab1(cv2_img_rgb, gray_img):
    ...
    with col2:
        ...
        plt.close(fig_hist_g)

        st.subheader("Histogram RGB")
        fig_rgb_h = plot_rgb_histogram(cv2_img_rgb)
        st.pyplot(fig_rgb_h)
        plt.close(fig_rgb_h)

        st.subheader("Matriks Pixel (Area 16×16)")
        patch = gray_img[:16, :16]
        fig_matrix = make_pixel_matrix_display(patch)
        st.pyplot(fig_matrix)
        plt.close(fig_matrix)

@safe_render
def render_tab2(cv2_img_rgb, gray_img, uploaded_file):
    ...
    with st.expander("ℹ️ Format Citra Digital"):
        ...
        """)
        st.info(f"Format file yang diupload: **{uploaded_file.type if uploaded_file else 'N/A'}**")
    ...
    center_val = int(gray_img[y_coord, x_coord])

    n4, n8 = {}, {}
    if y_coord > 0: n4["Atas (N)"] = int(gray_img[y_coord - 1, x_coord])
    if y_coord < gray_img.shape[0] - 1: n4["Bawah (S)"] = int(gray_img[y_coord + 1, x_coord])
    if x_coord > 0: n4["Kiri (W)"] = int(gray_img[y_coord, x_coord - 1])
    if x_coord < gray_img.shape[1] - 1: n4["Kanan (E)"] = int(gray_img[y_coord, x_coord + 1])
    n8 = dict(n4)
    if y_coord > 0 and x_coord > 0: n8["Atas-Kiri (NW)"] = int(gray_img[y_coord - 1, x_coord - 1])
    if y_coord > 0 and x_coord < gray_img.shape[1] - 1: n8["Atas-Kanan (NE)"] = int(gray_img[y_coord - 1, x_coord + 1])
    if y_coord < gray_img.shape[0] - 1 and x_coord > 0: n8["Bawah-Kiri (SW)"] = int(gray_img[y_coord + 1, x_coord - 1])
    if y_coord < gray_img.shape[0] - 1 and x_coord < gray_img.shape[1] - 1: n8["Bawah-Kanan (SE)"] = int(gray_img[y_coord + 1, x_coord + 1])

    with col_n1:
        st.markdown("**4-Neighbourhood**")
        neighbor_data_4 = {"Posisi": ["Pusat"] + list(n4.keys()), "Nilai Pixel": [center_val] + list(n4.values())}
    ...
    with col_n2:
        ...
        neighbor_data_8 = {"Posisi": ["Pusat"] + list(n8.keys()), "Nilai Pixel": [center_val] + list(n8.values())}
...
@safe_render
def render_tab3(cv2_img_rgb, gray_img):
    ...
    with col_g1:
        ...
        if "Rotasi" in selected_ops:
            ...
            h_w, w_w = cv2_img_rgb.shape[:2]
            center = (w_w // 2, h_w // 2)
            M = cv2.getRotationMatrix2D(center, angle, 1.0)
            rotated = cv2.warpAffine(cv2_img_rgb, M, (w_w, h_w), borderMode=cv2.BORDER_REPLICATE)
    ...
    with col_g2:
        if "Cropping" in selected_ops:
            ...
            h_h, w_w = cv2_img_rgb.shape[:2]
            x1 = st.number_input("X awal", 0, w_w - 1, 0, key="crop_x1")
            y1 = st.number_input("Y awal", 0, h_h - 1, 0, key="crop_y1")
            x2 = st.number_input("X akhir", 0, w_w - 1, w_w // 2, key="crop_x2")
            y2 = st.number_input("Y akhir", 0, h_h - 1, h_h // 2, key="crop_y2")
            if x1 < x2 and y1 < y2:
        ...
        if "Scaling (Resize)" in selected_ops:
            ...
            st.caption(f"Dimensi asli: {h_h}×{w_w} → Dimensi baru: {new_h}×{new_w}")
            show_result = True
            result_img = scaled
...
@safe_render
def render_tab5(cv2_img_bgr, cv2_img_rgb, gray_img):
    ...
    **Alur Segmentasi:**
    ...
    axes_comp[1].imshow(otsu_test, cmap="gray")
...
def main():
    ...
    **Menggunakan Metode Otsu Thresholding dan Operasi Morfologi**
    ...
    with tab1:
        render_tab1(cv2_img_rgb, gray_img)

    with tab2:
        render_tab2(cv2_img_rgb, gray_img, uploaded_file)

    with tab3:
        render_tab3(cv2_img_rgb, gray_img)

    with tab4:
        render_tab4(gray_img)

    with tab5:
        render_tab5(cv2_img_bgr, cv2_img_rgb, gray_img)
